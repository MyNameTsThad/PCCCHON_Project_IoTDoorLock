...
char auth[] = "8TCGpfc9LHE6HyFlnYKpeEzGZlsFUc8u";
...
const char* ssid = "Watcharapong_2.4G";
const char* password = "0868190813";
...
void takePhoto(){
    ...
    LINE.notifyPicture("urls", "http://"+local_IP+"/capture?_cb="+(String)randomNum);
    delay(1000);
}
void setup() {
    Serial.begin(115200);
...
void loop() {
    ...
    if(digitalRead(BUTTON)==HIGH){
        ...
        delay(1000);
        digitalWrite(LED,LOW);
        LINE.notify("Doorbell Pressed");
        delay(10000);
    }
    if(matchFace==false){
        LINE.notify("Instruder Detected");
        takePhoto();
    }
    delay(10000);
}
